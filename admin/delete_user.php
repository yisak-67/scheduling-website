...
$result=mysqli_query($conn,"select * from users where User_id='$id'")or die(mysqli_error);
$row=mysqli_fetch_array($result);
$f=$row['FirstName']." ".$row['LastName'];

mysqli_query($conn,"delete from users where User_id='$id'")or die(mysqli_error());

mysqli_query($conn,"INSERT INTO history (data,action,date,user)VALUES('$f', 'Delete User', NOW(),'$user_name')")or die(mysqli_error());
